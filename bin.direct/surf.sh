...
s_set_write_proper_uri() { # uri , file
...
"_SURF_URI_RAW")
    ...
    ;;
"_SURF_URI")
    sel=`tac $bmarks 2> /dev/null | $dmenu -fn "$font" -p "SURF: [dgtwuy[,e]*]:"`
    [ -z "$sel" ] && exit
    ...
    *)
    ...
    [ $save -eq 1 ] && (s_set_prop _SURF_GO "$uri"; s_write_f $histfile"$sel")
    # try to set the uri only if it is a success
    [ $save -eq 2 ] && s_set_write_proper_uri "$uri" "$histfile"
    ;;
*)
    echo Unknown xprop
    ;;
